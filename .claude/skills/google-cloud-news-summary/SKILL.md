...
対象アップデートのリスト (ステップ 4 の結果)
  ↓
オーケストレーター: RSS 取得 → パース → フィルタリング → 重複チェック
  ↓
┌─ Subagent A (アップデート A) ────────┐  ┌─ Subagent B (アップデート B) ────────┐
│  5.1 Release Notes 詳細取得          │  │  5.1 Release Notes 詳細取得          │
│  5.2 MCP ドキュメント検索            │  │  5.2 MCP ドキュメント検索            │
│  5.3 料金情報取得                    │  │  5.3 料金情報取得                    │
│  5.4 Before/After コンテキスト収集   │  │  5.4 Before/After コンテキスト収集   │
│  5.5 関連サービス調査                │  │  5.5 関連サービス調査                │
│  5.6 レポート作成 + アーキテクチャ図 │  │  5.6 レポート作成 + アーキテクチャ図 │
└──────────────────────────────────────┘  └──────────────────────────────────────┘
  ↓                                        ↓
  ... (すべての subagent が完了するまで並列実行)
```

**直接実行モード (手動実行時)**

手動で実行する場合は、従来通り順次処理も可能。
